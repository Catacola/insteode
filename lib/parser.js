 ...
 * @param {Function} callback(error,hexMessage)
 **/
var compose = exports.compose = function(msgOptions, callback){
  if(!us.has(msgOptions,"to")){
    if(callback){
      callback(new Error("'To' field not specified"));
      return;
    }
  }
  ...
  var cmd = getInsteonCommand(msgOptions.command);
  if(cmd === undefined){
    if(callback){
      callback(new Error("Unknown command: " + msgOptions.command));
    }
    return;
  }
  ...
  var hexCommand = composeStandardCommand(address,flagHex,cmd.hex,msgOptions.cmd2);

  if(callback){
    callback(null, hexCommand);
  }
  return;
};

/**
 * Retrieves insteon command object
 *
 * @param {String} cmdName
 **/

var getInsteonCommand = function(cmdName){
  var cmd = us.findWhere(insteonCommands,{name: cmdName});
 ...
 **/
var composeStandardCommand =  function(deviceHex,msgFlagHex,cmd1Hex,cmd2Hex){
