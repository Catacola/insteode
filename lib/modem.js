...
Modem.prototype.parseIncoming = function(data){
  ...
  if(curMessage.length < targetLength){
    ...
    curMessage += data.toString('hex');
    this.receivedMessages[this.receivedMessages.length-1] = curMessage;
    //check to see if message is complete
    if(curMessage.length === targetLength){
      us.each(us.where(this.activeCommands, {to: curMessage.slice(3,9)}), function(activeCmd){
        if(curMessage.slice(0,4) === "0262"){
    ...
    this.receivedMessages.push(data.toString('hex'));
  }
};
